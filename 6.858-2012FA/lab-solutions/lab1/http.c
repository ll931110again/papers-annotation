...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len)
{
    ...
    *sp2 = '\0';
    ...
    url_decode_n(reqpath, sizeof(reqpath), sp1);

    if ((size_t)(envp - env) + 20 + strlen(reqpath) >= 8192)
...
const char *http_request_headers(int fd)
{
    ...
    for (;;)
    {
        ...
        *colon = '\0';
        ...
        url_decode_n(value, sizeof(value), sp);
...
void dir_join(char *dst, size_t dstlen, const char *dirname, const char *filename) {
    snprintf(dst, dstlen, "%s%s%s", dirname,
             (dstlen > 0 && dirname[strlen(dirname) - 1] != '/') ? "/" : "",
             filename);
}
    ...
    default:
        ...
        while (1) {
            ...
            if (!statusprinted && strncasecmp("Status: ", buf, 8) == 0) {
                ...
                if (ret < 0 || (size_t)ret >= headerslen) {
...
{
    url_decode_n(dst, (size_t)-1, src);
}

void url_decode_n(char *dst, size_t dstlen, const char *src)
{
    size_t i = 0;

    for (;;)
    {
        if (dstlen != (size_t)-1 && i >= dstlen - 1)
        {
            dst[i] = '\0';
            break;
        }
        ...
        if (src[0] == '%' && src[1] && src[2])
        {
            ...
            dst[i++] = (char)strtol(&hexbuf[0], 0, 16);
            src += 3;
        }
        else if (src[0] == '+')
        {

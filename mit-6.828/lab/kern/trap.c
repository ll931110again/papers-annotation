...
trap_init(void)
{
	...
	SETGATE(idt[IRQ_OFFSET + IRQ_TIMER], 0, GD_KT, handler_irq0, 0);
	SETGATE(idt[IRQ_OFFSET + IRQ_KBD], 0, GD_KT, handler_irq1, 0);
	SETGATE(idt[IRQ_OFFSET + IRQ_SERIAL], 0, GD_KT, handler_irq4, 0);
	SETGATE(idt[IRQ_OFFSET + IRQ_SPURIOUS], 0, GD_KT, handler_irq7, 0);
	SETGATE(idt[IRQ_OFFSET + IRQ_IDE], 0, GD_KT, handler_irq14, 0);
	SETGATE(idt[IRQ_OFFSET + IRQ_ERROR], 0, GD_KT, handler_irq19, 0);
...
trap_init_percpu(void)
{
	...
	c->cpu_ts.ts_esp0 = (uint32_t) (uintptr_t) (percpu_kstacks[cpunum()] + KSTKSIZE);
	c->cpu_ts.ts_ss0 = GD_KD;
	gdt[(GD_TSS0 >> 3) + cpunum()] = SEG16(STS_T32A, (uint32_t) (&c->cpu_ts),
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 0) {
		if (fault_va >= ULIM)
			panic("page fault in kernel at %08x\n", fault_va);
		cprintf("page fault at %08x in kernel environment\n", fault_va);
		env_destroy(curenv);
		return;
	}
	...
	if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP)
		fault_stack = (void *) (tf->tf_esp - 4 - sizeof(struct UTrapframe));
	else
		fault_stack = (void *) (UXSTACKTOP - sizeof(struct UTrapframe));

	user_mem_assert(curenv, fault_stack, sizeof(struct UTrapframe), PTE_W);
	...
	memcpy(fault_stack, &utf, sizeof(struct UTrapframe));

	curenv->env_tf.tf_esp = (uintptr_t) fault_stack;

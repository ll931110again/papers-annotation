...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		if (i == 0)
			continue;
		if (i >= npages_basemem && i < IOPHYSMEM / PGSIZE)
			continue;
		...
		if (i >= EXTPHYSMEM / PGSIZE &&
		    i < ((uintptr_t)boot_alloc(0) - KERNBASE) / PGSIZE)
			continue;
		pages[i].pp_ref = 0;
...
{
	const char *start, *end, *addr;
	pte_t *pte;

	start = va;
	end = start + len;
	if (start >= (const char *)ULIM || start >= end) {

...
static int read_map(int fd, void **blk, size_t offset);
...
spawn(const char *prog, const char **argv)
{
	...
	// Copy shared library state before setting up the new address space.
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		if (!(ph->p_flags & ELF_PROG_FLAG_WRITE)) {
			void *blk;
			uintptr_t seg_va = ph->p_va;
			size_t seg_memsz = ph->p_memsz;
			off_t seg_offset = ph->p_offset;
			size_t j;
			int delta;

			perm |= PTE_SHARE;
			if ((delta = PGOFF(seg_va))) {
				seg_va -= delta;
				seg_memsz += delta;
				seg_offset -= delta;
			}
			for (j = 0; j < seg_memsz; j += PGSIZE) {
				if ((r = read_map(fd, &blk, seg_offset + j)) < 0)
					goto error;
				if ((r = sys_page_map(0, blk, child,
						      (void *) (seg_va + j), perm)) < 0)
					goto error;
				if ((r = sys_page_unmap(0, (void *) UTEMP)) < 0)
					goto error;
			}
		} else if ((r = map_segment(child, ph->p_va, ph->p_memsz,
...
static int
read_map(int fd, void **blk, size_t offset)
{
	int r;
	...
	if ((r = sys_page_alloc(0, (void *) UTEMP, PTE_P | PTE_U | PTE_W)) < 0)
		return r;
	if ((r = readn(fd, (void *) UTEMP, PGSIZE)) < 0)
		return r;
	*blk = (void *) UTEMP;
	return 0;
}
